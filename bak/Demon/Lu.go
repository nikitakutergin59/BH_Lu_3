...
func LCF(w http.ResponseWriter, r *http.Request) {
	...
	LCF_Body := struct {
		...
		InLCF_ID:     fmt.Sprintf("%s", req_TIL.TIL_ID),
		InLCF_Result: fmt.Sprintf("%s", resultCalc),
	}
	// кодируем JSON
	...
	otvetResp, err := http.Post("http://localhost:8080/inLCF", "application/json", bytes.NewBuffer(ilcfJSON))
	if err != nil {
