...
import (
	...
	tokens_BH "github.com/nikitakutergin59/BH_Lu_3/bak/token"
)
...
func Calcualtor(tokens []tokens_BH.Token, expr *Expression_BH, md *MemoryData) (string, error) {
	task_result_slice, err := OpenParent(tokens, expr, md) // openAllParent
	if err != nil {
	...
	tokensOTD, err := tokens_BH.Tokenize_BH(openTheDoor)
	if err != nil {
	...
	for _, resultMAD := range afteMAD_Slice {
		for i := 0; i < len(tokensOTD); i++ {
			if tokensOTD[i].Type == tokens_BH.TOKEN_OPERATOR && (tokensOTD[i].Value == "*" || tokensOTD[i].Value == "/") {
				...
				// Заменяем подвыражение на результат
				expression = strings.Replace(expression, subExpression, resultMAD, 1)
				log.Printf("Calculator: заменил подвыражение %s на результат %s", subExpression, resultMAD)

				// Обновляем токены
				tokensOTD = append(tokensOTD[:i-1], tokensOTD[i+2:]...) // Удаляем обработанные токены
				break
			}
		}
	}
	...
	MAD := expr.Expression
	tokensMAD, err := tokens_BH.Tokenize_BH(MAD)
	if err != nil {
		return "", fmt.Errorf("ошибка повторной токенизации: %w", err)
	}
	...
	for _, resultASS := range afterAAS_slice {
		for i := 0; i < len(tokensMAD); i++ {
			if tokensMAD[i].Type == tokens_BH.TOKEN_OPERATOR && (tokensMAD[i].Value == "+" || tokensMAD[i].Value == "-") {
				...
				expression = strings.Replace(expression, subExpression, resultASS, 1)
...
func SendTaskAgent(task *Task_BH) error {
	body := struct {
		...
		TIL_ID:    fmt.Sprintf("%s", task.Task_ID),
	...
	resp, err := http.Post("http://localhost:8081/LCF", "application/json", bytes.NewBuffer(JSON))
	if err != nil {
